...
#define NSPECIES        40         /* max # of species */
...
#define NGENE           19000        /* max # of loci; originally 17500 */
...
struct CommonInfo {
   ...
   char *spname[NS], seqf[512], Imapf[512], treef[512], outf[512], mcmcf[512];
   char checkpointf[512], concatf[512], locusratef[512], modelparaf[512], heredityf[512];
   char oldconP[NS * 2 - 1];
...
/* sptree.nseqsp is working space, copied from data.nseqsp[locus], which holds
   ...
   sptree.pptable[i][j] = 1 if pop j is ancestral to i and = 0 otherwise.
...
struct DATA {
   ...
   int nseqerr, iseqerr[NSPECIES], diploid, ngene;
...
struct DATA { /* locus-specific data and gene tree information, used in lnpG */
   ...
   int root[NGENE + 1], conP_offset[NGENE], *pose[NGENE], *Imap, diploid;
   unsigned char *z[NGENE][NS], *nHdiploid[NGENE];
...
int DiploidResolution(FILE *fout, int locus);
...
int main(int argc, char*argv[]) {
...
#endif
   char VerStr[64] = "Version 4, May 2017";
   FILE *fout;
   int i, k = 5;
   ...
   noisy = 3;
   com.sizespace = max2(1000000, data.ngene*sptree.nspecies * 2) * sizeof(double);  /* check ipop size */
...
int GetOptionsSimulation(char *ctlf)
{
   ...
   char *optstr[] = { "seed", "noisy", "seqfile", "treefile", "Imapfile", "modelparafile", "concatfile", "diploid",
                     "species&tree", "loci&length", "migration", "sequenceerror", "alpha_locusrate",
         ...
         for (i = 0, t = 0, pline = line; i < lline&&line[i]; i++)
            ...
            for (iopt = 0; iopt < nopt; iopt++) {
               if (strncmp(opt, optstr[iopt], 8) == 0) {
                  ...
                  case (7): sscanf(pline + 1, "%d", &data.diploid);   break;
                  case (8):
   ...
   if (data.diploid) {
      for (i = 0; i < sptree.nspecies; i++)  sptree.nseqsp[i] *= 2;
      com.ns *= 2;
      if (com.ns > NS)
...
int GetOptions(char *ctlf)
{
   int nopt = 26, lline = 4096, locfields[1000], iopt, i, is, ierror;
   char line[4096], *pline, opt[32], *comment = "*#", *seqerrstr = "0EF";
   char *optstr[] = { "seed", "noisy", "seqfile", "Imapfile", "outfile", "mcmcfile", "diploid", "checkpoint",
                     "BayesFactorBeta", "speciesdelimitation", "speciestree", "speciesmodelprior", "species&tree",
         ...
         for (i = 0, t = 0, pline = line; i < lline&&line[i]; i++)
            ...
            for (iopt = 0; iopt < nopt; iopt++) {
               if (strncmp(opt, optstr[iopt], 12) == 0) {
                  ...
                  case (6): sscanf(pline + 1, "%d", &data.diploid);   break;
                  case (7): sscanf(pline + 1, "%d", &com.checkpoint);   break;
                  case (8): sscanf(pline + 1, "%lf", &BFbeta);  /* beta for marginal likelihood */
                  case (9):
                  ...
                  case (10):
                     ...
                     break;
                  case (11): sscanf(pline + 1, "%d", &sptree.SpeciesModelPrior);   break;
                     break;
                  case (12):
                     if ((sptree.nspecies = com.ns = (int)t) > NSPECIES)
                        error2("raise NSPECIES in bpp.c & recompile?");
                  ...
                  case (16): sscanf(pline + 1, "%lf%lf %c", &data.theta_prior[0], &data.theta_prior[1], &ch);
                     sptree.NoTheta = (toupper(ch) == 'E' ? 0 : 1);
                     break;
   ...
   if (data.theta_prior[0] <= 2 || (sptree.nspecies > 1 && data.tau_prior[0] <= 2))
   ...
   if (data.maxns > NS) error2("raise NS in source file");
...
int ReadSpeciesTree(FILE* fctl, char *currline)
{
   ...
   */
   ...
   if (sptree.nspecies == 1) {
      ...
      for (i = 0; i < sptree.nnode; i++) {
         if (i != sptree.root && sptree.nodes[sptree.nodes[i].father].age < sptree.nodes[i].age) {
            printf("\nsptree node ages wrong: %.6f < %.6f.", sptree.nodes[sptree.nodes[i].father].age, sptree.nodes[i].age);
   ...
   sptree.M = (double*)malloc(sptree.nnode*sptree.nnode * sizeof(double));
   if (sptree.M == NULL) error2("oom M");
   memset(sptree.M, 0, sptree.nnode*sptree.nnode * sizeof(double));

   if (com.simulation) {
      for (i = 0, com.ns = 0; i < sptree.nspecies; i++)
         com.ns += sptree.nseqsp[i];
      /* data.diploid may not have been read by now, so com.ns is updated later. */
      if (com.ns > NS)
         error2("raise NS in bpp.c & recompile");
   }
   else {
...
int ReadMigrationMatrix(FILE *fctl, char *pline)
{
   /* Rates for impossible ancestor-descendent migrations are set to 0.
      ...
      in the same epoch.
   */
   int s21 = sptree.nspecies * 2 - 1, i, j, needtheta, status = 0, nMresets = 0;
   ...
   for (i = 0; i < s21; i++) {
      ...
      for (j = 0; j < s21; j++)
         fscanf(fctl, "%lf", &sptree.M[i*s21 + j]);
   }
   ...
   for (i = 0; i < s21; i++) {
      ...
      for (j = i; j < s21; j++) {
         ...
         if (i == j) {
            if (sptree.M[i*s21 + j] > 0) nMresets++;
            sptree.M[i*s21 + j] = sptree.M[j*s21 + i] = 0;
         }
         else if (sptree.pptable[i][j] || sptree.pptable[j][i] || Agei > Adadj || Agej > Adadi) {
            if (sptree.M[i*s21 + j] > 0) nMresets++;
            if (sptree.M[j*s21 + i] > 0) nMresets++;
            if (sptree.M[i*s21 + j] > 0 || sptree.M[j*s21 + i] > 0)
               printf("\nmigration between %s and %s impossible...", sptree.nodes[i].name, sptree.nodes[j].name);
            sptree.M[i*s21 + j] = sptree.M[j*s21 + i] = -1;
         }
      }
   }
   ...
   for (i = 0, FPN(F0); i < s21; i++, FPN(F0)) {
      ...
      for (j = 0; j < s21; j++) {
         printf(" %9.4f", sptree.M[i*s21 + j]);
      }
   }

   for (i = 0; i < s21; i++)
      for (j = 0; j < s21; j++)
         if (sptree.M[i*s21 + j] < 0)  sptree.M[i*s21 + j] = 0;
   for (i = 0; i < sptree.nspecies; i++) {
      for (j = 0, needtheta = 0; j < s21; j++)
         if (sptree.M[j*s21 + i]) needtheta = 1;
...
int CoalescentMigration(void)
{
   ...
   */
   ...
   int npop = sptree.nspecies, ipop[NSPECIES], *sons, s21 = sptree.nspecies * 2 - 1;
      ...
      for (; ; ) {
         ...
         if (sptree.migration)
            ...
            for (i = 0, M = 0; i < npop; i++) {
               for (j = 0, Mi[i] = 0; j < npop; j++)
                  Mi[i] += nin_G[i] * sptree.M[ipop[j] * s21 + ipop[i]] / sptree.nodes[ipop[i]].theta * 4;
               M += Mi[i];
            }
         ...
         if (r < C) {  /* coalescent in pop i of lineages k1 & k2 */
            ...
            for (j = 0; j < npop - 1; j++)      /* duplicated calculation! */
               if (r < (y += nin_G[i] * sptree.M[ipop[j] * s21 + ipop[i]] / sptree.nodes[ipop[i]].theta * 4))
                  break;
...
int MakeDiploid(unsigned char *z[], int *ns, int ls)
{
   /* This takes ns sequences in z[ns][] and produces ns/2 sequences, with heterozygotes.
      memory in z[ns/2,...,ns-1] is released.
      Sequences are already encoded.
   */
   int i, j, k, h;
   unsigned char b[2], c[2], seqname[64], *pch;

   if (*ns % 2 != 0) error2("There should be an even number of sequences?");

   for (i = 0; i < *ns / 2; i++) {
      for (h = 0; h < ls; h++) {
         b[0] = z[i * 2][h];  b[1] = z[i * 2 + 1][h];
         if (b[0] == b[1])
            z[i][h] = b[0];
         else {
            c[0] = min2(b[0], b[1]);  c[1] = max2(b[0], b[1]);
            if (c[0] == 0 && c[1] == 1)         z[i][h] = 5;   /* TC = Y */
            else if (c[0] == 2 && c[1] == 3)    z[i][h] = 6;   /* AG = R */
            else if (c[0] == 1 && c[1] == 2)    z[i][h] = 7;   /* CA = M */
            else if (c[0] == 0 && c[1] == 3)    z[i][h] = 8;   /* TG = K */
            else if (c[0] == 1 && c[1] == 3)    z[i][h] = 9;   /* CG = S */
            else if (c[0] == 0 && c[1] == 2)    z[i][h] = 10;  /* TA = W */
         }
      }
   }
   *ns /= 2;
   for (j = 0, k = 0; j < sptree.nspecies; j++) {
      ...
      for (i = 0; i < sptree.nseqsp[j] / 2; i++, k++)
...
*/
   ...
   if (sptree.nspecies != 3 || com.ns != 3) error2("should not be here.");
   if (t0 < tau1) {
...
void ClockSpeciesNodeRates(int snode, double snoderates[])
{
   ...
   for (i = 0; i < sptree.nodes[snode].nson; i++) {
      ison = sptree.nodes[snode].sons[i];
      if (sptree.nodes[snode].age == 0) 
...
void RelaxedClockBranchLengths(double snoderates[])
{
   ...
   */
   ...
   if (com.clock == 2) {  /* independent gamma rates  */
      for (is = 0; is < sptree.nnode; is++)
         snoderates[is] = rndgamma(data.a_branchrate) / data.a_branchrate;
   }
   else {                 /* autocorrelated gamma rates  */
      snoderates[sptree.root] = rndgamma(data.a_branchrate) / data.a_branchrate;
      ClockSpeciesNodeRates(sptree.root, snoderates);
   }
   if (debug) {
      matout(F0, snoderates, 1, sptree.nnode);
      printGtree(1);
   }
   for (i = 0; i < tree.nnode; i++) {
      nodes[i].branch = 0;
      ...
      tdad = nodes[dad].age;
      ipop = nodes[i].ipop;
      is = sptree.nodes[ipop].father;
      if (debug) printf("\nbranch %d (%.6f - %.6f)\n", i, t, tdad);
      for ( ; is!=-1 && tdad>sptree.nodes[is].age; is = sptree.nodes[is].father) {
         nodes[i].branch += (sptree.nodes[is].age - t)*snoderates[ipop];
         if (debug) printf("\tin pop %d, add (%.6f - %.6f)*%.6f\n", ipop, sptree.nodes[is].age, t, snoderates[ipop]);
...
void SimulateData(void)
{
   ...
   if (com.seqf[0]) {
      ...
      if (sptree.nspecies > 1) printf("Imap file is %s\n", com.Imapf);
   ...
   if (com.concatf[0]) {
      ...
      for (i = 0; i < (data.diploid ? com.ns / 2 : com.ns); i++) {
   ...
   for (locus = 0; locus < nloci; locus++) {
      ...
      if (fseq || ftree) {
         for (j = 0, k = 0; j < sptree.nspecies; j++) {
            ...
            if (data.diploid)
               for (i = 0; i < sptree.nseqsp[j]; i++, k++)
                  sprintf(com.spname[k], "%s%d%c^%s", seqname, i / 2 + 1, 'a' + i % 2, sptree.nodes[j].name);
            else
               for (i = 0; i < sptree.nseqsp[j]; i++, k++)
                  sprintf(com.spname[k], "%s%d^%s", seqname, i + 1, sptree.nodes[j].name);
         }
      ...
      if (com.clock == 2 || com.clock == 3) {
         ...
         for (j = 0; j < sptree.nnode; j++) fprintf(fmodelpara, " %.6f", snoderates[j]);
      }
...
//            if (com.model == 0) PatternWeightJC69like();
   ...
   if (sptree.nspecies == 3 && com.ns == 3 && data.diploid == 0) {
...
int GetRootTau(void)
{
   ...
   */
   ...
   if (data.diploid) printf("renovate GetRootTau, especially for diploid data.\n");
   sptree.roottau = data.tau_prior[1] / (data.tau_prior[0] - 1) * 4;
   if (mcmc.usedata && !data.diploid) {
...
int ReadSeqData(char *seqfile, char *locusratef, char *heredityf, FILE*fout, char cleandata, int *ipop)
{
   ...
   */
   ...
   int s = sptree.nspecies, locus, i, j, is, lname = 100;
   ...
   if (s > 1) {
      ...
      data.Imap = (int*)malloc(maxind * sizeof(int));
      data.Indnames[0] = (char*)malloc(maxind * lname * sizeof(char));
      if (data.Imap == NULL || data.Indnames[0] == NULL) error2("oom Imap & Indnames");
      memset(data.Imap, 0, maxind * sizeof(int));
      memset(data.Indnames[0], 0, maxind * lname * sizeof(char));
      ...
      for (i = 0; i < maxind; i++) {
         ...
         for (j = 0; j < s; j++)
            if (strcmp(line, sptree.nodes[j].name) == 0) {  /* ind i is from pop j. */
               data.Imap[i] = j;
               break;
            }
         if (j == s) {
   ...
   /***** GIBBON, 2017.10.8 **/
// int conf[50000] = { 0 }, ss[3] = {1, 3, 6}, h;
int conf[50000] = { 0 }, h;
char f3sname[512] = "3s.txt", f3sIDname[32] = "3s.Imap.txt", *p;
char *confstr[3] = {"123", "113", "223"};
int sID[3][3] = { { 0, 1, 2 }, { 0, 0, 2 },{ 1, 1, 2 } };
FILE *f3s, *f3sID;

// yt: 141119
// printf("species IDs (e.g., 1 3 6)? ");
// scanf("%d%d%d", ss+0, ss + 1, ss + 2);
// printf("species IDs: %d %d %d\n", ss[0], ss[1], ss[2]);
ss[0]--; ss[1]--; ss[2]--;

strcpy(f3sname, com.seqf);
p = strstr(f3sname, ".txt");
if(p==NULL) p = f3sname;
sprintf(p, ".%s%s%s.txt", sptree.nodes[ss[0]].name, sptree.nodes[ss[1]].name, sptree.nodes[ss[2]].name);
sprintf(f3sIDname, "3s%s%s%s.ID.txt", sptree.nodes[ss[0]].name, sptree.nodes[ss[1]].name, sptree.nodes[ss[2]].name);

f3s = gfopen(f3sname, "w");
f3sID = gfopen(f3sIDname, "w");
for (i = data.ngene / 2; i < data.ngene; i++)
   conf[i] = ((i + 1) < (data.ngene * 3) / 4 ? 1 : 2);
for (i = 0; i < data.ngene; i++) {
   j = (int)(data.ngene*rndu());
   int it = conf[i];  conf[i] = conf[j];  conf[j] = it;
}
//for (i = 0; i < data.ngene; i++) printf("%2d", conf[i]);  printf("\n");
...
/***** GIBBON, 2017.10.8 **/
      ...
      /* Determine the ind ID (j or data.Indnames[j]) and ipop (is) for tips at the locus.
         Seqname is not preserved in the program.  */
      for (i = 0; i < s; i++) data.nseqsp[locus][i] = 0;  /* this is counted twice for diploid data */
      for (i = 0; i < com.ns; i++) {
         if (sptree.nspecies == 1)
            is = 0;
         else {
            ...
            for (j = 0; j < nind; j++)
               if (strcmp(iname, data.Indnames[j]) == 0) break;
            if (j == nind) {
               printf("Individual label %s not recognised.", iname);
               error2("Please fix the Imap file.");
            }
            else
               is = data.Imap[j];  /* sequence i is from individual ind from species is. */
         }
         if (data.diploid)  ipop[i * 2 + 0] = ipop[i * 2 + 1] = is;
         else               ipop[i] = is;
      ...
      for (i = 0; i < s; i++) printf(" %2d", data.nseqsp[locus][i]);
      printf(" ), %s", strc(12, ' '));
      printf("\n%5d sites, %3d patterns, %s\r", com.ls, com.npatt, (com.cleandata ? "clean" : "messy"));

      if (data.diploid) {
         /* calculate data.nseqsp[locus] & rewrite com.spname[], using ipop, for diploid data */
         for (i = 0; i < s; i++) data.nseqsp[locus][i] = 0;
         for (i = 0; i < com.ns * 2; i++) {
            is = ipop[i];  data.nseqsp[locus][is] ++;
            sprintf(com.spname[i], "%s%d\0", sptree.nodes[is].name, data.nseqsp[locus][is]);
         }
         DiploidResolution(fout, locus);     /* this changes ns to ns*2. */
         fprintf(fout, "\ncleandata = %d\n", data.cleandata[locus]);
      }

      // yt 281121: skip loci if not enough sequences
      int enough_sqs = 1;
      printf("\n");
      for (i = 0; i < s; i++) {
         printf(" %2d", data.nseqsp[locus][i]);
      }
      printf("\n");
      // for (i = 0; i < 3; i++) printf(" %d", ss[ sID[conf[locus]][i] ]);
      for (i = 0; i < 3; i++) printf(" %d", data.nseqsp[locus][ss[sID[conf[locus]][i]]]);
      printf("\n");

      for (i = 0; i < 3; i++) {
         enough_sqs *= data.nseqsp[locus][ss[sID[conf[locus]][i]]] > 1;
      }
      printf("enough_sqs: %d\n", enough_sqs);

      if (enough_sqs) {
      // yt 281121: new indent from here
/***** GIBBON, 2017.10.8 **/
         // fprintf(f3sID, "%d (%s)", locus + 1, confstr[conf[locus]]);
         char keep[250] = {0};
         printf("com.ns: %d \n", com.ns);
         if (com.ns > 250) error2("increase keep size & recompile?");  // yt 281121
         // char keep[com.ns] = {0};  // illegal in C
         // for (j = 0; j < com.ns; j++)  printf("%2d ", keep[j]);      FPN(F0);

         int k, it;
         for (i = 0; i < 3; i++) {
            /* sample the k-th sequence from ss[i], which is sequence j. */
            int sIDt = ss[ sID[conf[locus]][i] ];  /* species ID from the species tree */
            k = (int)(data.nseqsp[locus][sIDt] * rndu());
            if (i==1 && (conf[locus] == 1 || conf[locus] == 2))
               k = (int)((data.nseqsp[locus][sIDt] - 1) * rndu());
            it = 0;
            for (j = 0; j < com.ns; j++) {
               if (ipop[j] != sIDt || keep[j]) continue;
               if (it++ == k)  { 
                  keep[j] = (char)1;
                  break;
               }
         ...
         // for (j = 0; j < com.ns; j++)  printf("%2d ", keep[j]);      FPN(F0);
         //// printf("species IDs: %d %d %d\n", ss[0], ss[1], ss[2]);
         for (j = 0, k = 0; j < com.ns; j++)  k += keep[j];
         printf("k: %d \n", k);
         if (k != 3) error2("k=3?");
         ...
         fprintf(f3sID, "\n");
         fprintf(f3s, "\n\n3  %d", com.ls);
         for (i = 0; i < 3; i++) {   /* 123, 113 or 223 */
            int sIDt = ss[sID[conf[locus]][i]];
            for (j = 0; j < com.ns; j++) {
               if (!keep[j] || ipop[j] != sIDt) continue;
               char *p = strchr(com.spname[j], '^');
               strcpy(p, p + 1);
               sprintf(p + strlen(p), "^%d\0", sID[conf[locus]][i]+1);
               fprintf(f3s, "\n%-20s  ", com.spname[j]);
               for(h=0; h<com.ls; h++) fprintf(f3s, "%c", com.z[j][h]);
               keep[j] = (char)0;
               break;
            }
         }
...
/***** GIBBON, 2017.10.8 **/
fprintf(f3sID, "\nlocus ID: sequence IDs for %s %s %s\n", sptree.nodes[ss[0]].name, sptree.nodes[ss[1]].name, sptree.nodes[ss[2]].name);
exit(0);
   ...
   /* maxns here is calculated from the seq file, whereas data.maxns is from the control file. */
   if (data.diploid) maxns *= 2;
   if (data.maxns < maxns) printf("# seqs in control file are too small and are reset.\n");
...
/* Notes about diploid data.  Ziheng, 14 January 2017.
   The original alignment (A0) at any locus has 1 sequence for each diploid individual and uses ambiguity
   characters YRMKSW to represent heterozygotes (Het).  N-? are allowed and treated as ?, but other
   ambiguities (HBVD) are not allowed right now (although this can be relaxed.  Note that it will be
   unfeasible to allow Y to be interpreted as both heterozygote and missing data).
   A0 is compresssed into hetero alignment A1 right after sequences are read.
   DiploidResolution() takes A1 as input.  It copies the number of site patterns in A1 into
   data.npattdiploid[locus] and the site-pattern counts into data.fpatt[locus][].  It then resolves some
   individuals/sites by constructing a H matrix and generates the expanded alignment A2, with the number
   of site patterns to be \sum_i 2^h_i, where i is a site pattern in A1 and h_i is its number of Hets.
   A2 is then JC-compressed into alignment A3, with the number of site patterns in data.npatt[locus].
   Columns in A2 are mapped to columns in A3 through data.pose[locus][]: if data.pose[locus][4] = 3 means
   the fifth pattern in A2 is the fourth pattern in A3.
   Calculation of conditional probability vectors uses A3, with data.npatt[locus] site patterns.  The log
   probability for the locus lnL sums over site patterns in A1, with the log probability for a column in
   A1 calculated as an average over 2^h_i resolved site patterns (which are in A2), the probabilities of
   which are retrieved from A3 through data.pose[locus][].
   After this pre-processing, the following are kept in the memory.  Other information (such as A0, A1, A2)
   is constructed and then destroyed.

   (i) data.npattdiploid[locus], data.fpatt[locus][], data.nHdiploid[] refer to A1 (het alignment).
   (ii) data.pose[locus][] maps columns in the expanded alignment A2 onto those in A3
   (iii) data.z[locus][], com.npatt refer to A3.
*/

int DiploidResolution(FILE *fout, int locus)
{
   /* This function processes the sequence alignments (A1) if the original data are diploid sequences.
   A forever loop resolves one individual per round.  In each one, a loop goes through the singleton sites
   to find the most variable, and given the most variable singleton site, find the least variable individual
   for arbitrary resolution.  Sequence alignments (het alignment A1) in data.z[locus] are encoded before
   calling this routine.
   nHsite[] = data.nHdiploid[] has Hets for sites in het alignment.
   nHind[] has Hets for individuals at the singleton sites only.
   This assumes nH < 255 at any site.  If there are >255 individuals, change to int data.nHdiploid[].
   */
   int ns = data.ns[locus], npatt1 = data.npatt[locus], nsingle, *singles, indchosen, y;
   int *index, *ispace, i1, i2, i, j, k, c, h, ht, site, ip, iround, *het;
   double *Hsingles;
   ...
   for (i = 0; i < ns; i++) {
      for (h = 0; h < npatt1; h++) {
         ...
         if (zt[i][h] < 11) {            /* YRMKSW */
            ...
            printf("%c not allowed in sequence %d at locus %d\n", zt[i][h], i + 1, locus + 1);
         else                            /* -N?*/
   ...
   if (debug > 1) {
      printf("\nH matrix, initial");
      for (i1 = 0, FPN(F0); i1 < ns; i1++, FPN(F0)) {
   ...
   if (debug > 1) {
      ...
      for (h = 0; h < ns; h++) printf(" %4d", nHind[h]);
      printf("\nH for sites\n");
      for (h = 0; h < npatt1; h++) printf(" %4d", nHsite[h]);
   }

   /*  loop to resolve one individual a time */
   for (iround = 0; iround < ns; iround++) {
      for (i = 0; i < ns; i++) if (!resolved[i]) break;    /* all indivs are resolved. */
      if (i == ns) break;
      ...
      for (i = 0; i < nsingle; i++) {             /* find most variable singleton site */
         site = singles[index[i]];
         for (j = 0, y = npatt1; j < ns; j++) {      /* find least variable indiv indchosen at site */
            if (resolved[j] || H[j*npatt1 + site] == 0) continue;
            if (y > nHind[j]) { y = nHind[j];  indchosen = j; }
         }
         if (y < npatt1) { /* We have found indchosen to resolve singleton site */
            H[indchosen*npatt1 + site] = -1;
            nHsite[site] --;
            resolved[indchosen] = 1;
            /* update singles list */
            if (--Hsingles[index[i]] == 0) {
               for (j = index[i]; j < nsingle - 1; j++) { singles[j] = singles[j + 1];  Hsingles[j] = Hsingles[j + 1]; }
               nsingle--;
            }
            break;
         }
      ...
      /* there is no need to resolve nonsingleton sites. */

      /* construt the alignment of expanded site patterns */
   for (h = 0; h < npatt1; h++) {
      if (nHsite[h])
   ...
   if (fout) {
      fprintf(fout, "\nH matrix\n");
      for (i1 = 0, FPN(F0); i1 < ns; i1++, FPN(fout)) {
         for (i2 = 0; i2 < npatt1; i2++)
      ...
      fprintf(fout, "\nnpatt after expansion in A2 = %3d\n", data.npatt[locus]);
   }
   ...
   for (h = 0, ht = 0; h < npatt1; h++) {
      if (debug > 3) {
         printf("\n\n****Pattern %2d in heter alignment: ", h + 1);
         for (i = 0; i < ns; i++) printf(" %c", BASEs[zt[i][h]]);
      }
      for (i = 0, k = 0; i < ns; i++) {
         c = zt[i][h];
         if (H[i*npatt1 + h] == 0)              /* homo */
            z1[i * 2 + 0] = z1[i * 2 + 1] = c;
         else if (H[i*npatt1 + h] == -1) {      /* fixed resolution */
            z1[i * 2 + 0] = (unsigned char)CharaMap[c][0];
            z1[i * 2 + 1] = (unsigned char)CharaMap[c][1];
         }
         else {
            het[k++] = i;
         }
      ...
      for (ip = 0; ip < (1 << nHsite[h]); ip++) {
         ...
         for (i = 0, j = ip; i < nHsite[h]; i++) {
            k = j % 2;  j /= 2;
            i1 = het[nHsite[h] - 1 - i];
            z1[i1 * 2 + 0] = CharaMap[zt[i1][h]][k];
            z1[i1 * 2 + 1] = CharaMap[zt[i1][h]][1 - k];
            if (debug > 3) printf(" %2d", k);
         }
         for (i = 0; i < ns * 2; i++)  data.z[locus][i][ht] = z1[i];
         ht++;

         if (debug > 3) {
            printf("  Site %2d: ", ht);
            for (i = 0; i < ns * 2; i++) printf("%2c", BASEs[z1[i]]);
         }
      }  /* for(ip) */
   }     /* for(h) */
   if (ht != data.npatt[locus]) error2("npatt incorrect");
   for (i = 0; i < ns; i++) free(zt[i]);


   /* setting things up to call PatternWeightJC69like() */
   com.ns = data.ns[locus] *= 2;
   com.npatt = com.ls = data.npatt[locus];
   for (i = 0; i < com.ns; i++)  com.z[i] = data.z[locus][i];
   com.pose = data.pose[locus] = (int*)malloc(data.npatt[locus] * sizeof(int)); /* this is useful */
...
int GetMem(int ipop[])
{
   ...
   */
   int s = sptree.nspecies, locus, i, j, s1, sizeGtrees;
   double *conP;
...
void FreeMem(void)
{
   ...
   free(data.T2h[0]);
   free(data.ncoal[0]);
   if (!sptree.NoTheta) free(data.lnpGi);
...
{
   /* This saves sptree, the gene trees, and the MCMC step lengths.
   */
   ...
   fwrite(&sptree, sizeof(sptree), 1, f);
...
int ReadMCMCstate(char *filename)
{
   ...
   */
   ...
   if (f == NULL) error2("file open error");
   fread(&sptree, sizeof(sptree), 1, f);
   for (i = 0, sizeGtrees = 0; i < data.ngene; i++)
      sizeGtrees += (data.ns[i] * 2 - 1) * sizeof(struct TREEN);
...
double CountCoalescentLocus(int locus)
{
   ...
   */
   int s = sptree.nspecies, ns = data.ns[locus], i, j, k, inode, ip, ipop, ipopdad, nt;
   ...
   for (i = 0; i < sptree.nspecies; i++) nin[i] = data.nseqsp[locus][i];
...
double lnpG_S(int locus)
{
   /* this calculates the probability of gene tree and coalescent times (using tree
      and nodes[]), given sptree (theta and tau).  This is used if(sptree.NoTheta == 0).
   */
   int s = sptree.nspecies, is, ip, nt;
   ...
   for (ip = 0; ip < sptree.npop; ip++) {
      ...
      if (nt > 0)   lnp += nt * log(2 / sptree.nodes[is].theta);
      if (T2h > 0)  lnp -= T2h / sptree.nodes[is].theta;
   }
...
{
   /* this calculates the probability of gene tree and coalescent times (using tree
      and nodes[]), given sptree (theta and tau), with theta integrated out.
   */
   int s = sptree.nspecies, is, ip, i, nt;
   double lnp = 0, T2h, a = data.theta_prior[0], b = data.theta_prior[1];
   double alnb, lnGa, y;
   ...
   for (locus = 0; locus < data.ngene; locus++) {
      ...
      for (inode = data.ns[locus], copytree = 1; inode < data.ns[locus] * 2 - 1; inode++) {
         ...
         if (lnacceptance >= 0 || rndu() < exp(lnacceptance)) {
            ...
            *lnpG += lnpGd;
            if (!sptree.NoTheta) data.lnpGi[locus] = lnpGinew;
            AcceptLocus(locus, 1);
            copytree = 0;
            if (debug == 2) printf(" Y\n");
         }
...
double UpdateGB_SPR(double* lnL, double* lnpG, double finetune)
{
   /* This removes a node (inode and its father) in the gene tree and regrafts it to a feasible branch
      ...
      (internal node on the species tree).  For example, sizeSnode[sptree.root]==com.ns.  This is used to
      ...
      (b) The population of the target node (ipop) must be ancestral to or the same as the population
          of inode: (sptree.pptable[species][ipop]==1).
   ...
   */
   ...
   int s = sptree.nspecies, sizeSnode[2 * NSPECIES - 1];  /* #descendents for ancestral species */
   ...
   for (locus = 0; locus < data.ngene; locus++) {
      ...
      if (sizeSnode[sptree.root] != com.ns) error2("sizeSnode");
      ...
      for (inode = 0, copytree = 1; inode < (mcmc.moveinnode ? com.ns * 2 - 1 : com.ns); inode++) {
         ...
         for (is = species; ; is = sptree.nodes[is].father)
            if (sizeSnode[is] > sizeinode) break;
         tb[0] = max2(tb[0], sptree.nodes[is].age);
...
double UpdateTau(double *lnL, double *lnpG, double finetune, double space[])
{
   ...
   */
   ...
   if (sptree.NoTheta) changetheta = 0;
   ...
   for (is = sptree.nspecies; is < sptree.nnode; is++) {
      ...
      lnacceptance = 0;
      if (is == sptree.root) /* tau_root ~ invgamma(a, b) */
      ...
      if (lnacceptance >= 0 || rndu() < exp(lnacceptance)) {
         ...
         for (locus = 0; locus < data.ngene; locus++) {
            ...
            if (!sptree.NoTheta) data.lnpGi[locus] = lnpGinew[locus];
...
double lnpriorSpeciesModel(int ndspecies)
{
   ...
   if ((sptree.SpeciesModelPrior & 1) == 1)                    /* prior 1 or 3: uniform rooted trees */
      p = 1 / CountLHsTree2();

   if (sptree.analysis == A11 && sptree.SpeciesModelPrior >= 2)  /* prior 2 or 3: uniform #species for A11 */
...
int SetSonNodeFlags(int ispecies, double tauU, char flag[])
{
   ...
   */
   int i, j, *sons = sptree.nodes[ispecies].sons;
   char debug = 0;

   for (i = 0; i < com.ns; i++) {
      if (sptree.pptable[nodes[i].ipop][sons[0]]) {
         flag[i] = 1;
   ...
   for (i = 0; i < com.ns; i++) {
      if (sptree.pptable[nodes[i].ipop][sons[1]]) {
         flag[i] = 2;
...
int UpdateSpeciesSplit(double *lnL, double* lnpG, double space[], double PrSplit)
{
   ...
   */
   ...
   thetai = sptree.nodes[is].theta;  /* not used if(sptree.NoTheta) */
   sons = sptree.nodes[is].sons;
   if (is == sptree.root)
      tauU = sptree.roottau*0.6;
   else
  ...
  */
   ...
   if (!sptree.NoTheta) {
      for (j = 0; j < 2; j++) {  /* theta_j & theta_k */
         ...
         /* prior on theta */
         lnacceptance += lnPDFinvGamma(sptree.nodes[sons[j]].theta, atheta, btheta);
      }
   }
   ...
   */
   ...
   if (lnacceptance >= 0 || rndu() < exp(lnacceptance)) { /* accept */
      ...
      *lnL += lnLd;
      *lnpG += lnpGd;
      if (sptree.NoTheta)  com.np++;
      else                com.np += 1 + (sptree.nodes[sons[0]].theta > 0) + (sptree.nodes[sons[1]].theta > 0);
   ...
   */
   ...
   if (lnacceptance >= 0 || rndu() < exp(lnacceptance)) { /* accept */
      ...
      *lnpG += lnpGd;
      ...
      copyCoalescent(1);
      if (!sptree.NoTheta) for (i = 0; i < 2; i++) sptree.nodes[sons[i]].theta = thetajk0[i];
   }
...
double mixing(double* lnL, double* lnpG, double finetune, double space[])
{
   ...
   */
   int accepted = 0, locus, is, i, k, ntau = sptree.nspecies - 1, ntheta;
   ...
   copyCoalescent(0);
   ntheta = (sptree.NoTheta ? 0 : sptree.npop);
...
int UpdateSpeciesTreeSPR(int NNIonly, double *lnL, double* lnpG, double space[])
{
   ...
   */
   ...
   /* probability of sampling sptree target branches in reverse move  */
   for (is = 0, ntarget = 0; is < sptree.nnode; is++) {
      dad = sptree.nodes[is].father;
      if (sptree.pptable[is][y] || sptree.nodes[is].age >= tau1 || sptree.nodes[dad].age <= tau1)
         continue;
      ...
      for (inode = com.ns; inode < tree.nnode; inode++) {
         ...
         if (tb[1] < tb[0] || t<tb[0] || t>tb[1]) {
            ...
            for (j = 0; j < 2 * sptree.nspecies - 1; j++, FPN(F0)) {
               ...
               printf("age %10.6g ", sptree.nodes[j].age);
            }
            printf("\ngene tree:\n");
            printGtree(1);
...
int DownSptreeSetTime(int inode)
{
   ...
   */
   if (sptree.nspecies <= 1) error2("fewer than 2 species?");
...
int GetInitials(void)
{
   /* This sets the initial values for starting the MCMC, and returns np, the
      ...
      See also collectx().
   */
   int np, is, i, j, k, ntheta, ntau = sptree.nspecies - 1, s = sptree.nspecies;
   double atau = data.tau_prior[0], btau = data.tau_prior[1];
   double atheta = data.theta_prior[0], btheta = data.theta_prior[1], y;

   ntheta = (sptree.NoTheta ? 0 : sptree.npop);
   /* initial theta's.  Some may be set to -1 later.
      if(sptree.NoTheta), theta are used to generate initial gene trees & then removed. */
   for (i = 0, k = 0; i < sptree.npop; i++)  /* invgamma prior  */
      sptree.nodes[sptree.pops[i]].theta = btheta / (atheta - 1)*(0.9 + 0.2*rndu());
   ...
   if (s > 1) {
      ...
      if (sptree.analysis == A10) {  /* A10: starting model chosen at random from nModels */
         ...
         */
         ...
         k = (int)(s*rndu());
         if (k < s - 1)
            for (i = s; i < s * 2 - 1; i++) sptree.nodes[i].age = !sptree.pptable[i][s + k];
      }
      if (sptree.nodes[s].age)          /* species tree root age */
         sptree.nodes[s].age = btau / (atau - 1)*(0.9 + 0.2*rndu());
   ...
   if (sptree.speciesdelimitation == 1 || sptree.speciestree == 1) {
      for (i = 0, ntheta = ntau = 0; i < sptree.nnode; i++) {
         ...
         if (!sptree.NoTheta) ntheta += (sptree.nodes[i].theta > 0);
...
int GetDmodelIndex(void)
{
   ...
   */
   ...
   model = (char*)bsearch(cmodel, sptree.DelimitationModels, sptree.nModels, sizeof(char)*s, (int(*)(const void*, const void*))strcmp);

   return (int)(model - sptree.DelimitationModels) / (sizeof(char)*s);
}
...
int InitializeDelimitationModel(int prinT, double priorPmodel[])
{
   ...
   */
   ...
   if (sptree.analysis != A10) error2("A10: should not be here?");
...
int PrintSmodel(FILE *fout, struct SMODEL *model, int printPhylogeny)
{
   ...
   for (i = 0; i < ns; i++) {
      ...
      while (*p) {      /* concatenate the spnames to make up a new spname */
         is = *p - 1;  /* pop is */
         k = (int)strlen(sptree.nodes[is].name);
...
int SummarizeA00_DescriptiveStatisticsSimpleBPP(FILE *fout, char infile[], int SkipColumns)
{
   ...
   int s = sptree.nspecies, nskip = SkipColumns + (sptree.NoTheta ? 0 : sptree.npop);
   FILE *fFigTree;
   ...
   if (sptree.NoTheta)
      for (j = 0; j < sptree.nnode; j++) sptree.nodes[j].theta = -1;
   ...
   for (j = s; j < sptree.nnode; j++) {       /* copy 95%HPD CI into nodes[].nodeStr for printing */
      ...
      if (nodes[j].label > 0)
         sprintf(nodes[j].nodeStr, "[&height_95%%_HPD={%.6g, %.6g}, theta=%.6g]\0", xHPD025[nskip + j - s], xHPD975[nskip + j - s], nodes[j].label);
      else
         sprintf(nodes[j].nodeStr, "[&height_95%%_HPD={%.6g, %.6g}]\0", xHPD025[nskip + j - s], xHPD975[nskip + j - s]);
   }
   if (sptree.nspecies > 1) {
...
int MCMC(FILE* fout)
{
   ...
   double mrootage = 0, Pspecies[NSPECIES] = { 0 };
   ...
   else if (sptree.analysis == A01)          /* screen output incorrect if NNI_ONLY */
      printf("\nStarting MCMC-SPR(%.0f%%)/Slider(%.0f%%)...\n", (1 - mcmc.pSlider) * 100, mcmc.pSlider * 100);
   else if (sptree.analysis == A11)            /* screen output incorrect if NNI_ONLY */
   ...
   if (com.checkpoint == 2) {
      ...
      if (sptree.NoTheta)  /* theta values were used to generate Gtrees & are now deleted. */
         for (j = 0; j < s * 2 - 1; j++) sptree.nodes[j].theta = -1;
   }
   for (locus = 0, lnpG = 0; locus < data.ngene; locus++) {
      UseLocus(locus, 0, mcmc.usedata, 0);
      CountCoalescentLocus(locus);
      if (!sptree.NoTheta) lnpG += (data.lnpGi[locus] = lnpG_S(locus));
   }
   if (sptree.NoTheta)
      lnpG = lnpG_S_NoTheta();
   ...
   for (ir = -mcmc.burnin; ir < mcmc.sampfreq*mcmc.nsample; ir++) {
      ...
      for (j = s, ndspecies = 1; j < s * 2 - 1; j++) if (sptree.nodes[j].age > 0)  ndspecies++;
      ...
      if (!sptree.NoTheta) Pjump[2] = (Pjump[2] * (nround - 1) + UpdateTheta(&lnpG, mcmc.finetune[2], com.space)) / nround;
      ...
      if (noisy && (k <= 500 || (ir + 1) % (k / 200) == 0)) {  /* printing on screen */
         ...
         if (sptree.analysis == A00) {          /* A00: print first few theta's, first 3 tau's */
            ...
            k = (sptree.NoTheta ? 0 : sptree.npop);
            for (j = k; j < min2(com.np, k + 3); j++) printf(" %6.4f", mx[j]);

            if (!sptree.NoTheta) {
               for (j = 0; j < min2(sptree.npop, 3); j++)  fprintf(fout, " %6.4f", mx[j]);
               fprintf(fout, " ");
            }
            k = (sptree.NoTheta ? 0 : sptree.npop);
            ...
            printf(" P[%d]=%5.3f  %6.4f", Bmodel + 1, sptree.pmodel[Bmodel] / nround, mrootage);

            fprintf(fout, " %2d %6.4f %s", com.np, (nroundRJ ? PjumpRJ / nroundRJ : 0), printDelimitationModel());
            fprintf(fout, " P[%d]=%5.3f  %6.4f", Bmodel + 1, sptree.pmodel[Bmodel] / nround, mrootage);
         }
         else if (sptree.analysis == A01) {     /* A01 */
            printf(" %5.3f  %6.4f", (nroundSPR ? PjumpSlider / nroundSPR : 0), mrootage);
            fprintf(fout, " %5.3f  %6.4f", (nroundSPR ? PjumpSlider / nroundSPR : 0), mrootage);
         }
         else if (sptree.analysis == A11) {     /* A11 */
            printf(" %2d %2d %6.4f %6.4f P(%d)=%5.3f  %6.4f", ndspecies, com.np,
               (nroundRJ ? PjumpRJ / nroundRJ : 0), (nroundSPR ? PjumpSlider / nroundSPR : 0),
               ndspeciesbest + 1, Pspecies[ndspeciesbest], mrootage);
            fprintf(fout, " %2d %2d %6.4f %6.4f P(%d)=%5.3f  %6.4f", ndspecies, com.np,
               (nroundRJ ? PjumpRJ / nroundRJ : 0), (nroundSPR ? PjumpSlider / nroundSPR : 0),
               ndspeciesbest + 1, Pspecies[ndspeciesbest], mrootage);
         }
